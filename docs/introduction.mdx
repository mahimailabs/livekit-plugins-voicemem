...
<CardGroup cols={2}>
  ...
  <Card title="Stays inside the turn" icon="gauge-high">
    Embedding takes 2.2 ms p50 in process, against 232 ms for an OpenAI round trip, and 2.6 ms
    p95 against 3.2 s.
  </Card>
  <Card title="No torch, no CUDA" icon="feather">
    The embedding model is 118 MB of ONNX. `onnxruntime` already ships with Silero and the turn
    detector, so for most agents it costs nothing new.
  </Card>
...
**Reading** happens inside the voice turn and makes no LLM call, and by default no network call
either. The query is embedded once, ranked in pgvector, and scored, with relevant traits fetched
concurrently. On the OpenAI backend it is also classified into one of seven life domains and
narrowed to those candidates first.
