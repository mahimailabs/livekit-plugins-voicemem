...
| `openai_api_key` | required | The two write-path calls. Not used for embeddings unless `embed_backend` is `openai`. |
...
| `embed_backend` | `auto` | `auto` follows the database: a fresh schema gets `local`, one already stamped with an OpenAI model keeps it. `local` or `openai` to decide yourself. |
| `embed_threads` | `1` | ONNX threads. The event loop and the audio pipeline are on this machine too. |
| `embed_allow_download` | `True` | Whether startup may fetch the model. Turn it off in production and pre-fetch with `voicemem-db fetch-model`. |
| `embed_model` | follows the backend | Ignored on the local backend, which reports its own. |
| `embed_dim` | follows the backend | 384 local, 1536 OpenAI. Part of the `vector(N)` column type, so it cannot change on a populated schema. |
| `slot_min_score` / `slot_margin` / `slot_max_slots` | `None` | Slot-routing overrides. Left alone, the container picks values suited to the embedding model. |
